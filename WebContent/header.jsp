		...
		<nav class="navbar navbar-expand-md navbar-dark bg-dark ">
			<div class="container">
				...
				<div class="collapse navbar-collapse" id="navbarsExample05">
					<ul class="navbar-nav ml-auto pl-lg-5 pl-0 navi">
						<li class="nav-item"><a class="nav-link active home"
							href="<%=path%>/index?command=main"><i
								class="fa fa-home fa-2x" aria-hidden="true"></i></a></li>
						<!-- 링크 -->
						<c:choose>
							<c:when
								test="${sessionScope.user == 1  && sessionScope.id !=null}">
								<!-- 로그인 할때 세션 넣어서 처리 -->
									...
									<div class="dropdown-menu" aria-labelledby="dropdown04">
										<c:choose>
											...
											<c:when test="${sessionScope.id !=null }">
												...
												<a class="dropdown-item"
													href="<%=path%>/user?command=logout">SIGN-OUT</a>
												<a class="dropdown-item"
													href="<%=path%>/user?command=MyPwCheck" id="mypage">MYPAGE
												</a>
											</c:when>
											<c:otherwise>
												<a class="dropdown-item"
													href="<%=path%>/user?command=loginmove">SIGN-IN</a>
												<!-- 로그인 하면 사라지고 Sign-out 이 나오기 -->
											</c:otherwise>
										</c:choose>
									</div></li>
							</c:when>
					...
					<div
						...
						<div class="col-md-8 text-center col-sm-12 ">
							<c:choose>
								<c:when test="${sessionScope.id !=null}">
									<div class="session">
										<span data-aos="fade-up" data-aos-delay="100"><b
											style="font-size: 1.3rem;">${sessionScope.id }様<br />おはようございます。
										</b></span>
									</div>
								</c:when>
								<c:otherwise>
