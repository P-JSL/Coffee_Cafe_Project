...
<section
	...
	<div class="container">
		...
		<c:choose>
			<c:when test="${sessionScope.user ==1 and sessionScope.user ==2 }">
				<!-- 로그인 한 userid로 변경 할 것 -->
				<div class="btn-group">
					<a href="notice?command=noticeWrite"
						class="pure-button pure-button-primary right">書く</a>
				</div>
			</c:when>
		</c:choose>
		<c:if test="${sessionScope.user ==2 }">
			...
			<div class="button-8">
				<div class="eff-8"></div>
				<a href="notice?command=noticeWrite"> 書く </a>
			</div>
		</c:if>
		<!-- Start Pagination -->
